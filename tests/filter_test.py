...
def test_filter_exclude():
    ...
    assert 'tests/mock.py' in exclude_filter(files)
    exclude_filter = Exclude(['mock.py'])
    assert 'tests/mock.py' in exclude_filter(files)

    exclude_filter = Exclude(['.'])
    assert exclude_filter(files) == []
    ...
    assert set(files) == set(exclude_filter(files))

    files = ['tests/test_dir/foo.py', 'tests/test_dir/bar.py',
             'tests/mock.py']
    exclude_filter = Exclude(['tests/mock.py'])
    assert set(exclude_filter(files)) == set(['tests/test_dir/foo.py',
                                              'tests/test_dir/bar.py'])
    exclude_filter = Exclude(['tests/test_dir'])
    assert set(exclude_filter(files)) == set(['tests/mock.py'])
